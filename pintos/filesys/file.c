...
void file_add_ref(struct file *file) {
  if (file != NULL) {
    file->ref_count++;
  }
}

int file_get_ref_count(struct file *file) {
  return file ? file->ref_count : 0;
}

int file_should_close(struct file *file) {
